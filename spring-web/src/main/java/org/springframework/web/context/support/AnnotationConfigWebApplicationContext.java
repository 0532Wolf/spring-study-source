...
public class AnnotationConfigWebApplicationContext extends AbstractRefreshableWebApplicationContext
	 ...
	 * 并扫描 {@link #scan（String...）} 指定的任何包。<p>对于 {@link #setConfigLocation（String）} 或 {@link #setConfigLocations（String[]）}
	 * 指定的任何值，首先尝试将每个位置加载为类，如果类加载成功，则注册 {@code BeanDefinition}，
	 * 如果类加载失败（即引发 {@code ClassNotFoundException}），假设该值是一个包并尝试扫描它以查找组件类。
	 * <p>启用默认的注释配置后处理器集，以便可以使用 {@code @Autowired}、{@code @Required} 和关联的注释。
	 * <p>配置类 Bean 定义使用生成的 Bean 定义名称进行注册，除非向构造型注释提供了 {@code value} 属性。
	 ...
	 * 为给定的 Bean 工厂构建一个 AnnotatedBeanDefinitionReader。这应该预先配置环境（如果需要），但还没有使用 BeanNameGenerator 或 ScopeMetadataResolver 配置。
	 * @param beanFactory the bean factory to load bean definitions into
	 * @since 4.1.9
	 * @see #getEnvironment()
